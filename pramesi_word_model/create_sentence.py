...
# Author : Steve Barlow
# $Id: create_sentence.py 14811 2017-01-26 13:13:29Z sjb $
# ******************************************************************************
...
vmodel = gs.models.Word2Vec.load(myDir + 'may2vec')
...
def create_sentence(input_text, seed=None, diversity=0.0):
    ...
    x_D = [vmodel[w] if w in vmodel else vmodel['.'] for w in correct_num_words]
    x_D = np.reshape(x_D, (1, sd_len, input_dim))
    
    output = input_text
    for iteration in range(100):
        preds = model.predict(x_D, verbose=0)[0]
        next_wcode = sample(preds, diversity)
        next_word = codedWord[next_wcode]
        if output != '' and next_word not in ['.', '!', '?', ',', u'\u201d']:
            output += ' '
        output += next_word
        x_D[0,0:-1] = x_D[0,1:]
